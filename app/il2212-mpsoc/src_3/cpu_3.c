...
int main(void) {
	  ...
	  *Begin_3=1;
	  ...
	  while(*Begin_3!=0){ delay(10);}     
	 


	  while( altera_avalon_mutex_trylock( mutex_0, ALT_CPU_CPU_ID_VALUE ) ){}
  	  if(altera_avalon_mutex_is_mine( mutex_0)) printf("CPU3 Handshake with CPU0 correct!\n");
	  
      *Begin_3=1;
	  
      altera_avalon_mutex_unlock( mutex_0 );
	  
	  alt_u8* memory=(alt_u8* )SHARED_ONCHIP_BASE+(32*14*3);
	  alt_u8 picture[32*3*11];
	  alt_u8 graypicture[32*11];
	  unsigned int *storeresult;
	  int i,j;

   	  alt_u8* ThirdResult=(alt_u8*)SHARED_ONCHIP_BASE+(32*32*3+13)+1;
	  alt_u8* ThirdFlag=(alt_u8*)SHARED_ONCHIP_BASE+(32*32*3+13)+5;
	  alt_u8* ThirdCoords_1=(alt_u8*)SHARED_ONCHIP_BASE+(32*32*3+13)+6;
	  alt_u8* ThirdCoords_2=(alt_u8*)SHARED_ONCHIP_BASE+(32*32*3+13)+7;
		
	  while(1){
			
			altera_avalon_mutex_lock( mutex_3, ALT_CPU_CPU_ID_VALUE );
			...
			*ThirdCoords_1=(alt_u8)storeresult[0];
			*ThirdCoords_2=(alt_u8)storeresult[1];
			*ThirdResult=(alt_u8*)storeresult[2];
